...
{
    size_t size1 = (*s1) ? ft_strlen(*s1) : 0;  // Gérer le cas NULL
    ...
    tmp[size1 + size2] = 0;
    *s1 = tmp;  // Maintenant on peut modifier le pointeur original
    return (1);
}

int str_append_str (char **s1, char *s2)
{
    return(str_append_mem(s1, s2, ft_strlen(s2)));  // Fix bug #5: passer s1 directement
}
...
void *ft_memmove(void *dest, const void *src, size_t n)  // Retourner void*
{
    ...
    if (dest > src)
    {
        i = n;  // Fix bug #6: utiliser n au lieu de ft_strlen
...
char *get_next_line(int fd)
{
    ...
    while (!tmp)  // Garder la condition originale mais corrigée
    {
    ...
    // On arrive ici seulement si tmp != NULL (on a trouvé '\n')
